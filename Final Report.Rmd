...
```
For two two different subsets I fitted two different multilevel models, the variables I use for multilevel are rating, score and rank, for the random effect is $(1 | source : genres)$. I also used two individual plots to check the model, “Residuals vs. Fitted” and “QQ Plot”. Furthermore the result of two fitted models shown below:
$$
\begin{aligned}
...
\end{aligned}
$$
where $n_j$ is random effect: $source:genres$
...
lmer_model_1 <- lmer(log(popularity) ~ rating + score + rank + (1 | source:genres), data = movie)
...
lmer_model_2 <- lmer(log(popularity) ~ rating + score + rank + (1 | source:genres), data = tv)
