...
& -0.935\cdot {ratingR+} - 0.97\cdot score - 0.0001\cdot rank + 0.195 \cdot ratingNone:score\\
& + 0.145 \cdot ratingPG:score - 0.397 \cdot ratingPG-13:score + 0.219 \cdot ratingR:score\\
&+ 0.017 \cdot ratingR+:score + +n_j + \epsilon\\
...
& -0.317\cdot {ratingR+} - 0.614\cdot score - 0.0001\cdot rank + 0.382 \cdot ratingNone:score\\
&+ 0.274 \cdot ratingPG:score -0.263 \cdot ratingPG-13:score + 0.369 \cdot ratingR:score\\
&+ 0.174 \cdot ratingR+:score+n_j + \epsilon\\
n_j \sim N(0,\sigma^2_a)
\end{aligned}
$$
where $n_j$ is random effect: $rating:episodes$  
...
```{r, fig.cap="Mean Value of Each Source", fig.height=4}
...
# 
# 
# rdfp1 <- ggplot(rdf1.1, aes(x = Rating,y = `random effect`, color = Rating)) +
#   geom_point() +
#   geom_text(aes(label = round(`random effect`,2)),vjust = 1.5, hjust = 1.5,size=3,check_overlap = TRUE,show_guide  = F) +
#   guides(label = FALSE )+labs(title='Movie',x='Rating')
# 
# rdfp2 <- ggplot(rdf2.1, aes(x = Rating,y = `random effect`, color = Rating)) +
#   geom_point() +
